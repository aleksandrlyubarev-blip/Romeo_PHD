...
const DOSKA_TV_QC_YAML = `name: DoskaTV Quality Control
description: Контроль качества и модерация объявлений раздела «Услуги» на DoskaTV — проверка фото и текста, маршрутизация спорных кейсов на ручную модерацию (HITL)

nodes:
  - id: listing_intake
    name: Разбор объявления
    type: qc_listing_intake
    prompt: "Нормализуй объявление раздела «Услуги»: заголовок, описание, категория услуги, география работы (точный адрес / город / регион / вся страна / онлайн), формат цены («за час»/«за консультацию»/«за сеанс»/«договорная»), контакты и мессенджеры, набор фотографий (количество и тип: портрет/процесс/результат). Явно перечисли отсутствующие или неоднозначные поля."

  - id: photo_authenticity
    name: Подлинность фото
    type: qc_photo_authenticity
    depends_on: [listing_intake]
    prompt: "Проверь фотографии на соответствие правилам DoskaTV: фото должно быть реальным и живым. Найди признаки нарушений — ИИ-генерация, стоковые картинки, надписи/водяные знаки/текстовые оверлеи/логотипы, тяжёлые фильтры и чрезмерная ретушь, рекламные баннеры/коллажи. По каждому фото дай вердикт с конкретными признаками."

  - id: photo_quality
    name: Качество и релевантность фото
    type: qc_photo_quality
    depends_on: [photo_authenticity]
    prompt: "Оцени качество и релевантность фото: есть ли реальное портретное фото специалиста (важно для доверия); есть ли фото рабочего процесса/результата там, где это применимо для категории (фотограф, мастер красоты, ремонтник, строитель, массажист, клинер, мастер по технике). Учитывай резкость, освещение, разрешение. Не штрафуй живые любительские фото за «нестудийность»."

  - id: content_compliance
    name: Соответствие текста
    type: qc_content_compliance
    depends_on: [listing_intake]
    prompt: "Проверь текст объявления: корректна ли выбранная категория услуги (соответствует тексту и фото); указана и осмысленна ли география работы; корректен ли формат цены; валидны ли контакты. Отметь спам, дубли, запрещённый/мошеннический контент и вводящие в заблуждение обещания."

  - id: trust_seo
    name: Доверие и SEO
    type: qc_trust_seo
    depends_on: [photo_quality, content_compliance]
    prompt: "Оцени уровень доверия объявления для каталога (полнота профиля, реальное фото, ясность услуги). Проверь SEO-готовность под органический трафик Google: естественно ли заголовок и описание отражают связку «услуга + город» (напр. «репетитор по математике в Хайфе») без переспама. Оцени пригодность текста к качественному переводу на иврит для доски מציאה."

  - id: moderation_decision
    name: Решение модерации
    type: qc_moderation_decision
    depends_on: [trust_seo]
    prompt: "Сформируй итоговое решение модерации: approve / reject / на ручную проверку. Перечисли причины и подготовь доброжелательный фидбэк-месседж специалисту: что именно исправить в фото и тексте, чтобы объявление прошло. Пограничные случаи (неочевидный ИИ/фильтр, спорная категория) отправляй на ручную модерацию, а не отклоняй автоматически."`;
...
export const BUXTER_TEMPLATES: readonly BuxterTemplate[] = [
  ...
  {
    id: "doska-tv-quality-control",
    name: "DoskaTV Quality Control",
    badge: "DoskaTV",
    sprint: "Trust & Safety",
    status: "active",
    nextFocus:
      "Подключить мультимодальную подачу реальных изображений в узлы проверки фото и калибровать пороги авто-аппрува vs ручной модерации.",
    summary:
      "Контроль качества раздела «Услуги» DoskaTV: проверка фото (без ИИ, без надписей/фильтров/рекламы) и текста, оценка доверия и SEO, маршрутизация спорных объявлений на ручную модерацию (HITL).",
    yaml: DOSKA_TV_QC_YAML,
    deliverables: [
      "Структурированный разбор объявления",
      "Проверка подлинности фото (ИИ/фильтры/надписи/реклама)",
      "Оценка качества и релевантности фото",
      "Проверка категории, географии и формата цены",
      "Оценка доверия + SEO и готовности к ивриту (מציאה)",
      "Решение модерации + фидбэк специалисту",
    ],
    tooling: ["Фото-модерация (Trust & Safety)", "Контроль текста и категорий", "Очередь ручной модерации (HITL)"],
    qualityGates: [
      "Фото реальное: без ИИ, надписей, тяжёлых фильтров и рекламы",
      "Категория, география и формат цены корректны",
      "Спорные кейсы уходят на ручную модерацию, а не отклоняются автоматически",
    ],
    phases: [
      {
        title: "Разбор и фото",
        owner: "DoskaTV QC",
        outcome: "Нормализовать объявление и проверить фотографии на подлинность, качество и релевантность услуге.",
      },
      {
        title: "Текст и доверие",
        owner: "DoskaTV QC",
        outcome: "Проверить категорию, географию, цену и контакты; оценить доверие, SEO и пригодность к переводу на иврит.",
      },
      {
        title: "Решение модерации",
        owner: "DoskaTV QC",
        outcome: "Вынести approve / reject / ручную проверку с причинами и доброжелательным фидбэком специалисту.",
      },
    ],
    handoff: "Выход — решение модерации по объявлению; пограничные кейсы попадают в очередь Consultations к живому модератору.",
  },
] as const;
